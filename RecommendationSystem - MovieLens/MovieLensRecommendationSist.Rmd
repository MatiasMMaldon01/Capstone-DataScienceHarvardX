 ...
 *1)* Searching for missing values.
 As we see, there is no missing values. This is really important because now we skip steps due to we dont have to fill this missing values.
```{r}
...
```
 *2)* Convert timestamp predictor into a human most readable format.
```{r}
edx$year <- edx$timestamp %>% as_datetime() %>% year()
edx$month <- edx$timestamp %>% as_datetime() %>% month()
```
 *3)* Extract the release date from title to a new predictor.
```{r}
